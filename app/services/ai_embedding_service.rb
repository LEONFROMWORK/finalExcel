...
class AiEmbeddingService
  ...
  def generate_embedding(text)
    ...
    # Use EmbeddingService for real OpenAI embeddings
    embedding_service = EmbeddingService.new(@api_key)
    embedding_service.generate_embedding(text)
  rescue => e
    Rails.logger.error "Failed to generate real embedding: #{e.message}"
    # Fallback to fake embedding in development/test only
