  ...
  module Services
    class CollectionService
      ...
      def read_file_data(file_path)
        ...
        if Rails.env.production? && File.exist?(file_path)
          # Production: Actually read the file
          begin
            ...
            Rails.logger.error "File read failed: #{e.message}"
            []
          end
        else
          # Development/Test: Return mock data
      ...
      def fetch_from_database(db_config)
        ...
        if Rails.env.production?
          # Production: Connect to external database
          begin
            # Example using ActiveRecord with different connection
            external_db = ActiveRecord::Base.establish_connection(db_config).connection
            results = external_db.execute("SELECT * FROM excel_tips LIMIT 100")
            external_db.close
            results.map(&:to_h)
          rescue => e
