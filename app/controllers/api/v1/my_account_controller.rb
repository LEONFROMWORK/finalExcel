  ...
  module V1
    ...
    class MyAccountController < Api::V1::ApiController
      ...
      def compile_personal_data(user)
        ...
        activities = user.user_activities.includes(:device_info)
        referral_rewards = ReferralReward.includes(:referral_code)
                                        .where("referrer_id = ? OR referred_id = ?", user.id, user.id)

        {
          ...
          credits_history: referral_rewards.map { |r| serialize_credit_transaction(r) }
        }
      end
    end
  end
