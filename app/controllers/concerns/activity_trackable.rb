...
module ActivityTrackable
  ...
  def filtered_params
    # 민감한 정보 제거 및 명시적 허용 키만 포함
    safe_params = params.except(:password, :password_confirmation, :token, :api_key)

    # 기본 허용 파라미터
    allowed_keys = [ :id, :action, :controller, :format, :page, :per_page, :sort, :order ]

    # 컨트롤러별 추가 허용 파라미터
    case controller_name
    when "excel"
      allowed_keys += [ :file_id, :analysis_type, :template_id ]
    when "chat_sessions"
      allowed_keys += [ :session_id, :message, :content ]
    when "vba_helper"
      allowed_keys += [ :error_message, :code_snippet, :context ]
    end

    # 명시적으로 허용된 키만 선택
    safe_params.to_unsafe_h.slice(*allowed_keys)
  end
